...
1. **Open the bench (0–8 seconds).** Point out the live Studio Next connection, chain ID, contract address, and release selector. If the address or RPC is unavailable, say so; do not switch to fixture state.
...
Before presenting, run `npm run verify:proof` and check that each link points to the currently deployed Studio Next contract and current Vercel evidence origin. If the proof is absent or fails, present only the local replay and label it as such.
