...
5. **Contrast HOLD (40–49 seconds).** Select `RATCHET-HOLD`. `PAUSE-OFF` is missing, so the candidate is blocked and the demo bond remains locked. The release shows its seven-day resolution date; after that date, anyone can resolve the hold under the rollback rule. HOLD means insufficient evidence, not a confirmed vulnerability.
