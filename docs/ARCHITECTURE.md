...
4. **Frontend:** Next.js reads version, statistics, releases, attempts, history, and adjudication receipts from the Studionet deployment in `frontend/lib/deployment.json`. Writes require an injected wallet on chain `61999` and finalized transactions. Public evidence can be reviewed without a wallet. The retired Studio Next deployment is kept as historical proof only.
5. **Deployment/proof scripts:** Studio scripts accept Studionet only, pin its stable runner and chain definition, write stable-specific deployment and receipt records, preflight public evidence byte-for-byte, seed deterministic demo proposals, and compare saved receipts to live state.
...
- Demo bond amounts are integer accounting units, not held assets. Only GenLayer transaction fees are paid for chain actions; the application does not add an estimate or fee-profile layer.
