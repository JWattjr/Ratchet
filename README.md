...
Ratchet is a GenLayer release-governance application. It freezes a release declaration, has GenLayer validators compare that declaration with hash-pinned replay evidence, and records an `ADVANCE`, `HOLD`, or `ROLLBACK` decision on-chain. The public demo targets stable Studionet (chain `61999`); Studio Next (`61997`) remains available for development. Ratchet does not execute EVM bytecode, move production assets, or perform a proxy upgrade or rollback.
...
- Studio Next (`61997`) for preview deployments and development
...
5. Set `$env:RATCHET_NETWORK = "studionet"`. Run `npm run deploy`, `npm run seed:demo`, `npm run test:integration`, `npm run verify:proof`, and `npm run profile:fees`. These scripts select Studionet’s chain definition and keep receipts separate from Studio Next.
6. Build and redeploy the frontend so it picks up the generated Studionet deployment manifest, verified proof snapshot, and measured fee profile. Remove the stale Vercel `NEXT_PUBLIC_RATCHET_ADDRESS` override, or update it to exactly match the verified Studionet address.
...
- `scripts/` — network-selected deployment, evidence preparation, seeding, proof verification/recovery, release gate, and receipt-based fee profiling.
